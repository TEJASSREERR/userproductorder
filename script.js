...
const routes = {
    '/': { component: 'Dashboard', handler: renderDashboard },
    '/users': { component: 'UserList', handler: renderUserList },
    '/users/new': { component: 'CreateUser', handler: renderUserForm },
    '/users/:id': { component: 'UserDetail', handler: renderUserDetail },
    '/users/:id/edit': { component: 'EditUser', handler: renderUserForm },
    '/products': { component: 'ProductList', handler: renderProductList },
    '/products/new': { component: 'CreateProduct', handler: renderProductForm },
    '/products/:id': { component: 'ProductDetail', handler: renderProductDetail },
    '/products/:id/edit': { component: 'EditProduct', handler: renderProductForm },
    '/orders': { component: 'OrderList', handler: renderOrderList },
    '/orders/new': { component: 'CreateOrder', handler: renderOrderForm },
    '/orders/:id': { component: 'OrderDetail', handler: renderOrderDetail },
    '/orders/:id/edit': { component: 'EditOrder', handler: renderOrderForm }
};
...
function router() {
    ...
    document.querySelectorAll('.nav-link').forEach(link => {
        ...
        if (linkRoute === currentPath || (currentPath === '' && link.dataset.route === '/')) {
    ...
    setTimeout(() => {
        route.handler(params, mainContent);
    }, 300);
}

/* ──────────────────────────────────────
   MOCK API HELPERS (Replace with real API later)
────────────────────────────────────── */
...
async function apiGet(endpoint) {
    ...
    if (endpoint.includes('/users/') && endpoint.length > 7) {
        const id = parseInt(endpoint.split('/')[2]);
        return mockUsers.find(u => u.id === id);
    }
    if (endpoint === '/users/') return mockUsers;
    
    if (endpoint.includes('/products/') && endpoint.length > 10) {
        const id = parseInt(endpoint.split('/')[2]);
        return mockProducts.find(p => p.id === id);
    }
    if (endpoint === '/products/') return mockProducts;
    
    if (endpoint.includes('/orders/') && endpoint.length > 8) {
        const id = parseInt(endpoint.split('/')[2]);
...
async function apiPost(endpoint, data) {
    ...
    if (endpoint === '/orders/') {
        const newOrder = { ...data, id: mockOrders.length + 1 };
...
async function apiPut(endpoint, data) {
    ...
    if (type === 'users') {
        ...
        if (index !== -1) mockUsers[index] = { ...data, id };
    }
    if (type === 'products') {
        const index = mockProducts.findIndex(p => p.id === id);
        if (index !== -1) mockProducts[index] = { ...data, id };
    }
    if (type === 'orders') {
        const index = mockOrders.findIndex(o => o.id === id);
        if (index !== -1) mockOrders[index] = { ...data, id };
    }
    return data;
}
...
async function renderDashboard(params, container) {
...
async function renderUserList(params, container) {
    const users = await apiGet('/users/');
...
async function renderUserForm(params, container) {
...
async function renderUserDetail(params, container) {
    const user = await apiGet(`/users/${params.id}/`);
    
    if (!user) {
        container.innerHTML = '<div class="text-center py-12 text-gray-500">User not found</div>';
        return;
    }
...
async function renderProductList(params, container) {
    const products = await apiGet('/products/');
...
async function renderProductForm(params, container) {
    const isEdit = window.location.hash.includes('/edit');
...
async function renderProductDetail(params, container) {
    const product = await apiGet(`/products/${params.id}/`);
    
    if (!product) {
        container.innerHTML = '<div class="text-center py-12 text-gray-500">Product not found</div>';
        return;
    }
...
async function renderOrderList(params, container) {
...
async function renderOrderForm(params, container) {
    const isEdit = window.location.hash.includes('/edit');
...
async function renderOrderDetail(params, container) {
    const [orders, users, products] = await Promise.all([
        apiGet('/orders/'),
        apiGet('/users/'),
        apiGet('/products/')
    ...
    if (!order) {
        container.innerHTML = '<div class="text-center py-12 text-gray-500">Order not found</div>';
        return;
    }
